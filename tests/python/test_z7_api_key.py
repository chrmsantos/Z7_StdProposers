...
class TestGetKeyFile(unittest.TestCase):
    def test_key_file_uses_data_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            with mock.patch.object(z7_api_key, "get_data_dir", return_value=tmp_path):
                key_file = z7_api_key._get_key_file()
                self.assertEqual(key_file.parent, tmp_path)
                self.assertEqual(key_file.name, "openrouter.key")
...
class TestValidateApiKey(unittest.TestCase):
    ...
    def test_short_key_is_invalid(self):
        self.assertFalse(self.mod._validate_api_key("sk-abc"))

    def test_valid_length_key(self):
        self.assertTrue(self.mod._validate_api_key("sk-" + "A" * 25))
...
class TestEncryptDecryptRoundtrip(unittest.TestCase):
    ...
    def test_write_and_read_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp:
            key_file = Path(tmp) / "openrouter.key"
            with mock.patch.object(self.mod, "_get_key_file", return_value=key_file):
                self.mod.write_api_key("sk-my-test-api-key-1234567890")
                self.assertTrue(key_file.exists())
                result = self.mod.read_stored_api_key()
                self.assertEqual(result, "sk-my-test-api-key-1234567890")
